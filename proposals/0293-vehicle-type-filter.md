...
For the Java Suite library complete this feature using two pull requests. The first one being all the protocol changes necessary and developer facing APIs from the SdlManager. The second pull request would focus on the SdlRouterService, SdlBroadcastReceiver, and SdlDeviceListener changes. This would allow the PM to verify the cross platform aspect of this feature first and ensure its inclusion into the next slate of releases no problem.
